...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadProducts() {
        try {
            ...
            if (error) {
                console.error('Error loading products:', JSON.stringify(error, null, 2));
                productsContainer.innerHTML = '<p class="text-center text-red-600">Erro ao carregar produtos. Tente novamente mais tarde.</p>';
                return;
            }
            if (!data || data.length === 0) {
                productsContainer.innerHTML = '<p class="text-center text-gray-600">Nenhum produto disponível no momento.</p>';
                return;
            }
            renderProducts(data);
            ...
            productsContainer.innerHTML = '<p class="text-center text-red-600">Erro inesperado ao carregar produtos.</p>';
        }
    }
